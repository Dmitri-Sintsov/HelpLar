 ...
 * @revision Carlos Guimaṛes <cvsguimaraes@gmail.com>
...
class CIDR {
	...
	public static function cidrToLongRange($cidr) {
		$range = array();
		$cidr = explode('/', $cidr);
	...
	public static function cidrToRange($cidr) {
		return array_map('long2ip', static::cidrToLongRange($cidr));
	}

	public static function inet_aton($strIP) {
		$lngIP = ip2long($strIP);
		if ($lngIP === false) {
			return false;
		}
		return ($lngIP < 0) ? $lngIP + 4294967296 : $lngIP;
	}
	
	/**
     * method cidrToList.
     * Returns an array of all IP addresses in a CIDR.
     * Usage:
     *     CIDR::cidrToList("127.0.0.128/32");
     * Result:
         *     array(2) {
         *       [0]=> string(11) "127.0.0.128"
         *       [1]=> string(11) "127.0.0.129"
         *     }
     * @param $cidr string CIDR block
     * @return Array of IP address in a CIDR.
     */
    public static function cidrToList($cidr) {
        $range = CIDR::cidrToRange($cidr);
